...
inline float  string_to_float(const string &str)   {
	return stof(str);
}
inline wx_to_float(const wxString &wx) {
	string str = static_cast<const char*>(wx.To8BitData());
	stringstream ss(str);
	float i;
	ss>> i;
	return i;
...
inline wx_to_int(const wxString &wx){
	string str = static_cast<const char*>(wx.To8BitData());
	stringstream ss(str);
	int i;
	ss>> i;
...
inline string int_to_str(const int &i) {
...
inline string float_to_str(const float &i) {
